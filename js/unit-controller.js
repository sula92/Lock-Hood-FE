$(document).ready(function () {
    loadDepartments(0);
    LoadAllDepsTODropDown();
});

function LoadAllDepsTODropDown() {
  var ajaxConfig = {
    method: "GET",
  ...
  $.ajax(ajaxConfig)
    .done(function (factories, status, jqXHR) {
      ...
      for (var i = 0; i < factories.length; i++) {
        var option = document.createElement("option");
        var factoriesDD = document.getElementById("dep");
...
$("#tblDeartments").delegate("tr", "click", function () {
  ...
  var departmentId = $(this).children("td:nth-child(3)").first().text();
  ...
  $("#id").val(id);
  $("#nam").val(name).focus();
  $("#factoryDropdown").val(departmentId);
...
async function update() {
  var id = $("#id").val();
  var name = $("#name").val();
  var departmentId = $("#factoryDropdown").val();
...
$("#tblDeartments").on("click", "tbody tr td:last-child i", function () {
  if (confirm("Are sure you want to delete this Unit ?")) {
